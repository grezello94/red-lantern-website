  ...
  if (printer && assignmentMode) {
    ...
    if (assignmentMode === 'edit') {
      ...
      if (grid && anchor) {
        ...
        typography.innerHTML = `<label>Font family<select id="printer-edit-font-family">${fonts.map((font) => `<option value="${esc(font)}" ${String(printer.fontFamily || 'Arial') === font ? 'selected' : ''}>${esc(font)}</option>`).join('')}</select></label><label>Body font size<select id="printer-edit-font-size">${[8,9,10,11,12,13].map((size) => `<option value="${size}" ${Number(printer.fontSize || 10) === size ? 'selected' : ''}>${size} pt</option>`).join('')}</select></label><label>Header font size<select id="printer-edit-header-size">${[12,13,14,15,16,17,18].map((size) => `<option value="${size}" ${Number(printer.headerFontSize || 15) === size ? 'selected' : ''}>${size} pt</option>`).join('')}</select></label><label class="printer-edit-check"><input id="printer-edit-header-bold" type="checkbox" ${printer.headerBold !== false ? 'checked' : ''}> Bold header</label><label class="printer-edit-check"><input id="printer-edit-footer-bold" type="checkbox" ${printer.footerBold ? 'checked' : ''}> Bold footer</label>`;
