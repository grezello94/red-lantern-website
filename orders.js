...
document.getElementById('operations-content')?.addEventListener('click', async (event) => {
  ...
  if (renamePrinter) { const printer=operationsConfig.printers.find((item)=>item.id===renamePrinter.dataset.renamePrinter); if (!printer) return; const name=prompt('Name this printer for your staff (for example: Kitchen Printer or Tandoor KOT).', printer.name || printer.deviceName || ''); if (name === null) return; const clean=String(name).trim().slice(0,60); if (!clean) { alert('Enter a printer name.'); return; } printer.name=clean; try { await saveOperations(); renderOperations(); } catch (error) { alert(error.message); } return; }
