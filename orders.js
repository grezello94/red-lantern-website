    ...
    if (assignmentMode === 'edit') {
      ...
      if (grid && anchor) {
        ...
        if (printer.type === 'bill') {
          ...
          preview.addEventListener('click', (event) => { const section=event.target.closest('[data-preview-target]'); const target=section?.dataset.previewTarget; if (!target) return; const input=target==='header'?document.getElementById('printer-edit-header'):target==='footer'?document.getElementById('printer-edit-footer'):document.getElementById(`printer-edit-${target}`); input?.scrollIntoView({ behavior:'smooth', block:'center' }); input?.focus({ preventScroll:true }); });
          updatePreview();
        }
