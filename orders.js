...
function renderTableView() {
  ...
  if (!moveKotItemsMode) content.querySelectorAll('.table-tile').forEach((tile) => {
    ...
    tile.replaceWith(wrap); wrap.append(tile, actions);
  });
...
document.addEventListener('click', (event) => {
  ...
  ['itemNameMinWidth','quantityColumnWidth','priceColumnWidth','amountColumnWidth'].forEach((key) => { const input=document.getElementById(`printer-edit-${key}`); if(input) printer[key]=Math.max(8,Math.min(220,Number(input.value)||0)); });
...
document.addEventListener('click', (event) => {
  ...
  if (printer && input) { const feed=Number(input.value); printer.kotBottomFeedLines=Number.isFinite(feed) ? Math.max(0, Math.min(12, feed)) : 3; }
}, true);
  ...
  if (printer && assignmentMode) {
    ...
    if (assignmentMode === 'edit') {
      ...
      if (grid && anchor) {
        ...
        if (printer.type === 'kot') { const centerControl=document.createElement('label'); centerControl.className='printer-edit-check'; centerControl.innerHTML=`<input id="printer-edit-kot-details-centered" type="checkbox" ${printer.kotDetailsCentered?'checked':''}> Center KOT details`; typography.querySelector('.printer-format-fields')?.append(centerControl); }
        if (printer.type === 'bill') { const columns=document.createElement('div'); columns.className='receipt-column-values'; columns.innerHTML=`<label>Item column width<input id="printer-edit-itemNameMinWidth" type="number" min="50" max="220" value="${Math.max(50,Number(printer.itemNameMinWidth)||110)}"></label><label>Qty column width<input id="printer-edit-quantityColumnWidth" type="number" min="25" max="60" value="${Math.max(28,Number(printer.quantityColumnWidth)||28)}"></label><label>Price column width<input id="printer-edit-priceColumnWidth" type="number" min="40" max="100" value="${Math.max(46,Number(printer.priceColumnWidth)||46)}"></label><label>Amount column width<input id="printer-edit-amountColumnWidth" type="number" min="52" max="120" value="${Math.max(60,Number(printer.amountColumnWidth)||60)}"></label>`; typography.querySelectorAll('.printer-format-fields')[2]?.append(columns); }
        if (printer.type === 'bill') {
          ...
          const updatePreview = () => {
            ...
            const usableWidth=Math.max(160,value('billingMainWidth',250)-8), itemWidth=Math.max(50,value('itemNameMinWidth',110)), quantityWidth=Math.max(28,value('quantityColumnWidth',28)), priceWidth=Math.max(46,value('priceColumnWidth',46)), amountWidth=Math.max(60,value('amountColumnWidth',60));
            preview.querySelectorAll('.rp-table').forEach((row) => row.style.gridTemplateColumns=`minmax(${Math.max(50,usableWidth-quantityWidth-priceWidth-amountWidth-12)}px,1fr) ${quantityWidth}px ${priceWidth}px ${amountWidth}px`);
            const previewContentLeft=12+(value('billingOuterLeft')/3), labelWidth=Math.max(50,usableWidth-quantityWidth-priceWidth-amountWidth-12); const positions={item:previewContentLeft+labelWidth,qty:previewContentLeft+labelWidth+quantityWidth+4,price:previewContentLeft+labelWidth+quantityWidth+priceWidth+8};
            paperPreview.querySelectorAll('[data-column-drag]').forEach((handle) => { handle.style.left=`${positions[handle.dataset.columnDrag]}px`; });
          ...
          paperPreview.querySelectorAll('[data-preview-drag]').forEach((handle) => handle.addEventListener('pointerdown', (event) => {
            event.preventDefault(); event.stopPropagation(); const edge=handle.dataset.previewDrag; const key={left:'billingOuterLeft',right:'billingOuterRight',top:'billingOuterTop',bottom:'billingOuterBottom'}[edge]; const input=typography.querySelector(`#printer-edit-${key}`); if (!input) return; const start=Number(input.value)||0; const origin=edge==='left'||edge==='right'?event.clientX:event.clientY;
            const move=(moveEvent) => { const distance=(edge==='left'||edge==='right'?moveEvent.clientX:moveEvent.clientY)-origin; const direction=edge==='left'||edge==='top'?1:-1; input.value=Math.max(0,Math.min(40,Math.round(start+(distance*direction*3)))); input.dispatchEvent(new Event('input',{bubbles:true})); };
          ...
          paperPreview.querySelectorAll('[data-column-drag]').forEach((handle) => handle.addEventListener('pointerdown', (event) => {
            event.preventDefault(); event.stopPropagation(); const key={item:'itemNameMinWidth',qty:'quantityColumnWidth',price:'priceColumnWidth'}[handle.dataset.columnDrag]; const input=typography.querySelector(`#printer-edit-${key}`); if (!input) return; const start=Number(input.value)||0; const origin=event.clientX;
            const move=(moveEvent) => { input.value=Math.max(Number(input.min)||0,Math.min(Number(input.max)||220,Math.round(start+(moveEvent.clientX-origin)))); input.dispatchEvent(new Event('input',{bubbles:true})); };
            const finish=() => { window.removeEventListener('pointermove',move); window.removeEventListener('pointerup',finish); };
            window.addEventListener('pointermove',move); window.addEventListener('pointerup',finish,{once:true});
          }));
        ...
        if (printer.type === 'kot') {
          const preview=document.createElement('aside'); preview.className='receipt-live-preview';
          ...
          typography.prepend(preview);
          const updateKotPreview=()=>{ const value=(key,fallback=0)=>{ const parsed=Number(typography.querySelector(`#printer-edit-${key}`)?.value); return Number.isFinite(parsed)?parsed:fallback; }; const pt=(points,min,max)=>Math.max(min,Math.min(max,points))*1.333; const paper=preview.querySelector('[data-kot-preview]'); const centered=!!document.getElementById('printer-edit-kot-details-centered')?.checked; const showSerial=!!document.getElementById('printer-edit-show-serial')?.checked; paper.style.setProperty('--receipt-font',typography.querySelector('#printer-edit-font-family')?.value||'Arial'); preview.querySelector('[data-kp-title]').style.fontSize=`${pt(value('kotTitleFontSize',15),10,22)}px`; preview.querySelector('[data-kp-title]').style.fontWeight=document.getElementById('printer-edit-header-bold')?.checked===false?'400':'800'; preview.querySelector('[data-kp-kot]').style.fontSize=`${pt(value('kotMetaFontSize',10),8,18)}px`; preview.querySelector('[data-kp-kot]').style.textAlign=centered?'center':'left'; preview.querySelector('[data-kp-customer]').style.fontSize=`${pt(value('kotMetaFontSize',10),8,18)}px`; preview.querySelector('[data-kp-customer]').style.textAlign=centered?'center':'left'; preview.querySelectorAll('[data-kp-item]').forEach((item,index)=>{ item.style.fontSize=`${pt(value('kotItemFontSize',12),8,20)}px`; item.style.marginBottom=`${Math.max(0,value('itemRowGap',5))/3}px`; item.textContent=`${showSerial?`${index+1}. `:''}${index===0?'2x Chicken Tandoori (Full)':'1x Cheese Garlic Naan'}`; }); preview.querySelectorAll('.rp-rule').forEach((rule)=>{rule.style.margin=`${Math.max(0,value('separatorGap',3))/2}px 0`;rule.style.height=`${Math.max(1,Math.min(4,value('separatorThickness',1)))}px`;}); preview.querySelector('[data-kp-customer]').style.display=document.getElementById('printer-edit-customer')?.checked===false?'none':''; };
          typography.querySelectorAll('input,select,textarea').forEach((input)=>input.addEventListener('input',updateKotPreview)); ['printer-edit-customer','printer-edit-show-serial','printer-edit-kot-details-centered'].forEach((id)=>document.getElementById(id)?.addEventListener('input',updateKotPreview));
          preview.addEventListener('click',(event)=>{const target=event.target.closest('[data-preview-target]')?.dataset.previewTarget; const input=target==='header'?document.getElementById('printer-edit-header'):target==='footer'?document.getElementById('printer-edit-footer'):target==='showCustomer'?document.getElementById('printer-edit-show-customer'):document.getElementById(`printer-edit-${target}`); input?.scrollIntoView({behavior:'smooth',block:'center'});input?.focus({preventScroll:true});}); updateKotPreview();
          const updateKotFeed=()=>{ const rawFeed=Number(typography.querySelector('#printer-edit-kotBottomFeedLines')?.value); const feed=Number.isFinite(rawFeed)?Math.max(0,rawFeed):3; const extra=Math.max(0,Number(document.getElementById('printer-edit-space')?.value)||0); const feedSpace=preview.querySelector('[data-kp-footer]'); preview.querySelector('[data-kot-preview]').style.minHeight='0'; feedSpace.textContent=''; feedSpace.style.display='block'; feedSpace.style.height=`${(feed+(extra*2))*14}px`; feedSpace.style.margin='0'; };
          typography.querySelectorAll('input,select').forEach((input)=>input.addEventListener('input',updateKotFeed)); document.getElementById('printer-edit-space')?.addEventListener('input',updateKotFeed); updateKotFeed();
          const updateKotTitle=()=>{ preview.querySelector('[data-kp-title]').textContent=String(document.getElementById('printer-edit-name')?.value||printer.name||'Kitchen').trim()||'Kitchen'; };
          document.getElementById('printer-edit-name')?.addEventListener('input',updateKotTitle); updateKotTitle();
          preview.addEventListener('click',(event)=>{const target=event.target.closest('[data-preview-target]')?.dataset.previewTarget; const input=target==='header'?document.getElementById('printer-edit-kotTitleFontSize'):target==='footer'?document.getElementById('printer-edit-kotBottomFeedLines'):event.target.closest('.rp-rule')?document.getElementById('printer-edit-separatorGap'):null; if (!input) return; event.stopImmediatePropagation(); input.scrollIntoView({behavior:'smooth',block:'center'}); input.focus({preventScroll:true});},true);
        }
...
document.getElementById('table-view-content')?.addEventListener('click', async (event) => {
  ...
  if (viewOrder) {
    ...
    counterCart=(Array.isArray(order.items)?order.items:[]).map((item)=>({ ...item, price:Number(String(item.price||0).replace(/[^0-9.]/g,'')) }));
    await openCounterOrder({ area:order.table_area || 'Dining', number:Number(order.table_number), orderId:order.id });
  ...
  if (printBill) {
    ...
    try {
      ...
      const marked=await fetch(`/api/orders/${encodeURIComponent(order.id)}/bill-printed`,{method:'POST'});
      const data=await marked.json().catch(()=>({}));
      if (!marked.ok) throw new Error(data.error || 'Bill printed, but the table could not be marked for settlement.');
      await loadOrders();
      renderTableView();
    } catch (error) {
