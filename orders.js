...
document.addEventListener('click', (event) => {
  ...
  ['billingMainWidth','billingOuterTop','billingOuterRight','billingOuterBottom','billingOuterLeft','billingItemBoxHeight','restaurantNameFontSize','headerFooterFontSize','dateBillFontSize','itemListingFontSize','grandTotalFontSize','grandTotalContentWidth','billHeaderHeight','billNameRowHeight','billDetailsHeight','billItemHeaderHeight','billSummaryHeight','billFooterHeight','itemNameMinWidth','serialColumnWidth','quantityColumnWidth','priceColumnWidth','amountColumnWidth','itemRowGap','separatorGap','separatorThickness','kotHeaderFontSize','kotTitleFontSize','kotMetaFontSize','kotItemFontSize','kotFooterFontSize'].forEach((key) => { const input=document.getElementById(`printer-edit-${key}`); if(input) printer[key]=Number(input.value); });
}, true);
  ...
  if (printer && assignmentMode) {
    ...
    if (assignmentMode === 'edit') {
      ...
      if (grid && anchor) {
        ...
        } else typography.innerHTML = `<div class="printer-format-intro"><span>Bill format</span><b>Every physical layout value below is saved per printer.</b></div><details class="printer-format-group" open><summary><span><b>Paper & margins</b><small>Controls receipt width and safe printing area</small></span><i>⌄</i></summary><div class="printer-format-fields">${field('billingMainWidth','Bill print width',309,'78.5 mm printable width for 80 mm paper. Reduce only if the right edge is cut off.')}${field('billingOuterLeft','Left outer space',0)}${field('billingOuterTop','Top outer space',0)}${field('billingOuterRight','Right outer space',0,'Increase only when content reaches beyond the right edge.')}${field('billingOuterBottom','Bottom outer space',0)}${field('billingItemBoxHeight','Minimum item row height',0)}</div></details><details class="printer-format-group"><summary><span><b>Text style</b><small>Font and hierarchy for the printed bill</small></span><i>⌄</i></summary><div class="printer-format-fields"><label>Font family<select id="printer-edit-font-family">${fonts.map((font) => `<option value="${esc(font)}" ${String(printer.fontFamily || 'Arial') === font ? 'selected' : ''}>${esc(font)}</option>`).join('')}</select></label>${field('restaurantNameFontSize','Restaurant name font size',14)}${field('headerFooterFontSize','Header / footer font size',13)}${field('dateBillFontSize','Date / bill box font size',13)}${field('itemListingFontSize','Item listing font size',10)}${field('grandTotalFontSize','Grand total font size',14)}${field('grandTotalContentWidth','Grand total content width',261,'261 = 66.2 mm. This creates the measured inset total row.') }<label class="printer-edit-check"><input id="printer-edit-header-bold" type="checkbox" ${printer.headerBold !== false ? 'checked' : ''}> Bold restaurant name</label><label class="printer-edit-check"><input id="printer-edit-footer-bold" type="checkbox" ${printer.footerBold ? 'checked' : ''}> Bold footer</label></div></details><details class="printer-format-group"><summary><span><b>Measured section heights</b><small>Minimum heights in 1/100 inch; item rows expand the receipt as needed</small></span><i>⌄</i></summary><div class="printer-format-fields">${field('billHeaderHeight','Header height',173,'4.4 cm reference height.')}${field('billNameRowHeight','Name row height',30,'0.76 cm reference height.')}${field('billDetailsHeight','Date / bill / token height',83,'2.12 cm reference height.')}${field('billItemHeaderHeight','Item-heading height',23,'0.59 cm reference height.')}${field('billSummaryHeight','Summary row height',26,'0.66 cm reference height.')}${field('billFooterHeight','Thank-you footer height',81,'2.06 cm reference height.')}</div></details><details class="printer-format-group"><summary><span><b>Item columns & spacing</b><small>Fine-tune the line-item layout</small></span><i>⌄</i></summary><div class="printer-format-fields">${field('itemNameMinWidth','Minimum item-name width',110,'The renderer protects this width and scales other columns if needed.')}${field('serialColumnWidth','Sr. no. column width',10,'Used only when item serial numbers are enabled.')}${field('quantityColumnWidth','Quantity column width',20)}${field('priceColumnWidth','Price column width',40)}${field('amountColumnWidth','Amount column width',55)}${field('itemRowGap','Item row gap',2)}${field('separatorGap','Separator gap',3)}${field('separatorThickness','Separator thickness',1)}</div></details>`;
