    ...
    if (assignmentMode === 'edit') {
      ...
      if (grid && anchor) {
        ...
        if (printer.type === 'kot') {
          ...
          preview.innerHTML=`<div><b>Live KOT preview</b><p>Click a ticket section to jump to its setting. Text, hierarchy, dividers and customer details update as you edit.</p></div><div class="receipt-preview-paper" data-kot-preview><div class="rp-center" data-kp-header data-preview-target="header">Kitchen · Red Lantern</div><div class="rp-center rp-name" data-kp-title data-preview-target="kotTitleFontSize">Tandoori Station</div><div class="rp-rule"></div><b data-kp-kot data-preview-target="kotMetaFontSize">KOT # 12</b><div data-kp-customer data-preview-target="showCustomer">Table: AC · 1<br>Guest: Walk-in customer</div><div class="rp-rule"></div><div data-kp-item data-preview-target="kotItemFontSize">2x Chicken Tandoori (Full)</div><div data-kp-item data-preview-target="kotItemFontSize">1x Cheese Garlic Naan</div><div class="rp-rule"></div><div class="rp-foot" data-kp-footer data-preview-target="footer">Thank you</div></div>`;
          typography.prepend(preview);
          const updateKotPreview=()=>{ const value=(key,fallback=0)=>Number(typography.querySelector(`#printer-edit-${key}`)?.value)||fallback; const pt=(points,min,max)=>Math.max(min,Math.min(max,points))*1.333; const paper=preview.querySelector('[data-kot-preview]'); paper.style.setProperty('--receipt-font',typography.querySelector('#printer-edit-font-family')?.value||'Arial'); preview.querySelector('[data-kp-header]').style.fontSize=`${pt(value('kotHeaderFontSize',12),8,20)}px`; preview.querySelector('[data-kp-header]').style.fontWeight=document.getElementById('printer-edit-header-bold')?.checked===false?'400':'800'; preview.querySelector('[data-kp-title]').style.fontSize=`${pt(value('kotTitleFontSize',15),10,22)}px`; preview.querySelector('[data-kp-kot]').style.fontSize=`${pt(value('kotMetaFontSize',10),8,18)}px`; preview.querySelector('[data-kp-customer]').style.fontSize=`${pt(value('kotMetaFontSize',10),8,18)}px`; preview.querySelectorAll('[data-kp-item]').forEach((item)=>item.style.fontSize=`${pt(value('kotItemFontSize',12),8,20)}px`); preview.querySelectorAll('.rp-rule').forEach((rule)=>{rule.style.margin=`${Math.max(0,value('separatorGap',3))/2}px 0`;rule.style.height=`${Math.max(1,Math.min(4,value('separatorThickness',1)))}px`;}); preview.querySelector('[data-kp-customer]').style.display=document.getElementById('printer-edit-show-customer')?.checked===false?'none':''; preview.querySelector('[data-kp-footer]').style.fontSize=`${pt(value('kotFooterFontSize',10),8,18)}px`; preview.querySelector('[data-kp-footer]').style.fontWeight=document.getElementById('printer-edit-footer-bold')?.checked?'800':'400'; preview.querySelector('[data-kp-header]').innerHTML=String(document.getElementById('printer-edit-header')?.value||'').replace(/\n/g,'<br>')||'Kitchen · Red Lantern'; preview.querySelector('[data-kp-footer]').innerHTML=String(document.getElementById('printer-edit-footer')?.value||'').replace(/\n/g,'<br>')||'Thank you'; };
          typography.querySelectorAll('input,select,textarea').forEach((input)=>input.addEventListener('input',updateKotPreview)); document.getElementById('printer-edit-header')?.addEventListener('input',updateKotPreview); document.getElementById('printer-edit-footer')?.addEventListener('input',updateKotPreview);
