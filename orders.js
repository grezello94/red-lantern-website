...
document.addEventListener('click', (event) => {
  ...
  ['billingMainWidth','billingOuterTop','billingOuterRight','billingOuterBottom','billingOuterLeft','billingItemBoxHeight','restaurantNameFontSize','headerFooterFontSize','dateBillFontSize','itemListingFontSize','grandTotalFontSize','serialColumnWidth','quantityColumnWidth','priceColumnWidth','amountColumnWidth','itemRowGap','separatorGap','itemsPerPage'].forEach((key) => { const input=document.getElementById(`printer-edit-${key}`); if(input) printer[key]=Number(input.value); });
}, true);
...
document.getElementById('view-table-kot')?.addEventListener('click', () => { const orderId=document.getElementById('view-table-kot').dataset.orderId, order=orderRecords.get(orderId), entries=operationKotHistory.get(orderId)||[]; if(!order)return; document.getElementById('view-kot-content').innerHTML=entries.length?entries.map((kot)=>`<section class="view-kot-ticket"><h3>KOT #${esc(kot.kot_number)} <small>${kot.created_at?new Date(kot.created_at).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}):''}</small></h3>${(kot.tickets||[]).flatMap((ticket)=>ticket.items||[]).map((item)=>{const index=(order.items||[]).findIndex((candidate)=>candidate.name===item.name&&String(candidate.portion||'')===String(item.portion||'')&&Number(candidate.quantity||0)>0);return `<div><b>${Number(item.quantity||0)}×</b> ${esc(item.name)}${item.portion?` · ${esc(item.portion)}`:''}<span>₹${Number(String(item.price||0).replace(/[^0-9.]/g,'')||0).toFixed(0)}</span>${index>=0?`<button type="button" class="view-kot-delete" data-view-kot-delete="${index}">Delete</button>`:''}</div>`;}).join('')}</section>`).join(''):'<p>No KOT has been sent for this table yet.</p>'; viewKotDialog.showModal(); });
viewKotDialog.addEventListener('click',async (event)=>{if(event.target.closest('.view-kot-close')){viewKotDialog.close();return;}const button=event.target.closest('[data-view-kot-delete]');if(!button)return;const orderId=document.getElementById('view-table-kot')?.dataset.orderId,order=orderRecords.get(orderId),index=Number(button.dataset.viewKotDelete);if(!order||!Number.isInteger(index)||!confirm('Delete this item from the active table bill?'))return;const quantities=(order.items||[]).map((item,itemIndex)=>itemIndex===index?0:Number(item.quantity||0));button.disabled=true;try{const response=await fetch(`/api/orders/${encodeURIComponent(orderId)}/items`,{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({quantities})});const data=await response.json().catch(()=>({}));if(!response.ok)throw new Error(data.error||'Unable to delete this item.');await loadOrders();viewKotDialog.close();}catch(error){alert(error.message||'Unable to delete this item.');button.disabled=false;}});
