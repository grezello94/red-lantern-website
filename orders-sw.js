const CACHE = 'red-lantern-orders-v16';
const ORDER_SHELL = [
  '/orders',
  '/orders.js?v=21',
  '/orders.css?v=7',
  '/orders-logo.css?v=7',
  '/orders-fixes.css?v=10',
...
self.addEventListener('install', (event) =>
  event.waitUntil(
    (async () => {
      await Promise.all(
        [...ORDER_SHELL, ...OFFLINE_DATA].map(async (url) => {
          try {
            await cacheResponse(url, await fetch(url, { credentials: 'same-origin' }));
          } catch {}
        })
      );
...
self.addEventListener('activate', (event) =>
  event.waitUntil(
    (async () => {
      await Promise.all(
        (await caches.keys())
          .filter((key) => key.startsWith('red-lantern-orders-') && key !== CACHE)
    ...
    (async () => {
      try {
        ...
        if (network.ok || network.status === 401 || network.status === 403 || !isOrdersShell) {
          ...
          if (url.pathname === '/api/orders' && network.ok)
            (await caches.open(CACHE)).put('/api/orders', network.clone());
          return network;
        }
      } catch {}
      ...
      return new Response(
        JSON.stringify({ error: 'Offline data is not available on this device yet.' }),
...
self.addEventListener('push', (event) => {
  let data = { title: 'New Direct Order', body: 'Open Direct Orders to view it.', url: '/orders' };
  try {
...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then((windows) => {
      const current = windows.find((client) => client.url.includes('/orders'));
      return current
        ? current.focus()
