...
app.post('/api/orders/counter', async (req, res) => {
  ...
  try {
    ...
    if (isDineIn) {
      const activeTable=await sql`SELECT id FROM direct_orders WHERE mode='table' AND table_area=${dineInArea} AND table_number=${dineInNumber} AND order_day=${orderDay}::date AND status IN ('saved','held','accepted','preparing','ready') LIMIT 1`;
...
app.post('/api/orders/:id/kots', async (req, res) => { try {
  ...
  for (const item of pending) {
    ...
    matchingRoutes.forEach((route)=>{ const printer=printers.find((candidate)=>candidate.id===route.printerId&&candidate.type==='kot'&&candidate.deviceName); if (!printer||printedBy.has(printer.id)) return; printedBy.add(printer.id); if(!groups.has(printer.id)) groups.set(printer.id,{ printerName:printer.deviceName, printerLabel:printer.name, items:[] }); groups.get(printer.id).items.push(item); });
  }
