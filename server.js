...
app.post('/api/orders/counter', async (req, res) => {
  ...
  try {
    ...
    if(source==='captain'&&expectedTableOrderId&&(!activeTable.length||activeTable[0].id!==expectedTableOrderId))return res.status(409).json({error:'This table changed while the order was being prepared. Open the table again, review it, then send the new round.'});
    if(source==='captain'&&!expectedTableOrderId&&activeTable.length)return res.status(409).json({error:'This table is now active. Open it again to review the current bill before adding a round.'});
