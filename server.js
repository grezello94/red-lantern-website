...
function allowPublicRequest(req, res, scope, limit, windowMs) {
  ...
  const previous = (publicRequestWindows.get(key) || []).filter((timestamp) => now - timestamp < windowMs);
  if (previous.length >= limit) {
    const retryAfter = Math.max(1, Math.ceil((windowMs - (now - previous[0])) / 1000));
    res.set('Retry-After', String(retryAfter));
    res.status(429).json({ error:'Too many requests. Please wait a moment and try again.' });
    logDiagnostic({ level:'warning', category:'security', message:'Public QR endpoint rate limit reached.', method:req.method, path:req.path, statusCode:429, ipHash:hashIp(req), userAgent:req.headers['user-agent'] || '', details:{ scope } });
    return false;
  }
  previous.push(now);
  publicRequestWindows.set(key, previous);
  if (publicRequestWindows.size > 5000) {
    for (const [entryKey, timestamps] of publicRequestWindows) if (!timestamps.some((timestamp) => now - timestamp < windowMs)) publicRequestWindows.delete(entryKey);
  }
  return true;
}
...
async function requireOrdersConsole(req, res, next) {
  const captainReadRoute = req.method === 'GET' && ['/api/orders', '/api/orders/operations', '/api/orders/menu', '/api/orders/availability','/api/captain/menu-insights'].includes(req.path);
  ...
  const captain=captainRoute && captainSession ? await getActiveCaptainSession(captainSession) : null;
  if (captain) { req.captain=captain; return next(); }
  if (captainRoute && captainSession) return res.status(401).json({ error:'Captain sign-in has expired. Sign in again.' });
...
async function ensureKotRoundStatusTable() {
  if (!sql) throw new Error('Orders database is not configured.');
  if (!kotRoundStatusTableReady) kotRoundStatusTableReady = sql`CREATE TABLE IF NOT EXISTS order_kot_round_status (order_id TEXT NOT NULL, kot_number INTEGER NOT NULL, printer_id TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'accepted', updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), PRIMARY KEY (order_id, kot_number, printer_id))`;
...
function normalizeAirMenu(body) {
  ...
  try {
    ...
    if (Array.isArray(parsed)) addonGroups = parsed.slice(0,100).map((group) => { const selection=group.selection==='multiple'?'multiple':'single',max=selection==='single'?1:Math.max(1,Math.min(20,Math.floor(Number(group.max)||1))),min=Math.min(max,Math.max(0,Math.min(20,Math.floor(Number(group.min)||0)))); return { id:String(group.id || crypto.randomUUID()).replace(/[^a-zA-Z0-9_-]/g,'').slice(0,60), name:String(group.name || '').trim().slice(0,80), displayName:String(group.displayName || '').trim().slice(0,100), min, max, selection, active:group.active!==false, options:(Array.isArray(group.options)?group.options:[]).slice(0,50).map((option)=>({name:String(option.name||'').trim().slice(0,80),price:Math.max(0,Math.min(100000,Number(option.price)||0)),dietary:option.dietary==='nonveg'?'nonveg':'veg'})).filter((option)=>option.name) }; }).filter((group)=>group.id&&group.name);
  } catch { addonGroups = []; }
...
app.post('/api/orders/counter', async (req, res) => {
  ...
  try {
    const { customerName, customerPhone, specialRequest, loyaltyPoints, tableArea, tableNumber, tableOrderId, items = [], action = 'submit', source = '' } = req.body || {};
    const captain=source==='captain'?await getActiveCaptainSession(req.get('X-Captain-Session')):null;
    if(source==='captain'&&!captain)return res.status(401).json({error:'Captain sign-in has expired. Sign in again.'});
    ...
    const clean = submitted.map((item) => {
      ...
      const style=/^(gravy|semi-gravy)$/i.test(String(item.style||'').trim())&&(source.gravyStyleAvailable||source.gravyAvailable||source.semiGravyAvailable)?String(item.style).trim():'';
      return { name:displayItemName(source.name).slice(0,100), category:String(source.category||'').slice(0,80), portion:String(item.portion||'').slice(0,40), style, note:String(item.note||'').trim().slice(0,80), quantity:Math.min(20,Number(item.quantity)||0), price:`₹${priceNumber(price)}`, availabilityKey:`${String(source.category||'').toLowerCase()}::${String(source.name||'').toLowerCase()}` };
    }).filter(Boolean);
    ...
    if(isDineIn&&captain&&captain.areas.length&&!captain.areas.includes(dineInArea))return res.status(403).json({error:'This table area is not assigned to your Captain account.'});
    const activeTable=isDineIn?await sql`SELECT id,items,total,daily_order_number,loyalty_points_redeemed,status FROM direct_orders WHERE mode='table' AND table_area=${dineInArea} AND table_number=${dineInNumber} AND order_day=${orderDay}::date AND status IN ('saved','held','accepted','preparing','ready') LIMIT 1`:[];
    const expectedTableOrderId=String(tableOrderId||'').trim().slice(0,120);
    const tableConflict=activeTable[0]?{id:activeTable[0].id,orderNumber:String(activeTable[0].daily_order_number).padStart(2,'0'),status:activeTable[0].status,items:Array.isArray(activeTable[0].items)?activeTable[0].items:[]}:null;
    if(source==='captain'&&expectedTableOrderId&&(!activeTable.length||activeTable[0].id!==expectedTableOrderId))return res.status(409).json({error:'This table changed while the order was being prepared. Review the offline order before merging it.',code:'table_changed',conflict:tableConflict});
    if(source==='captain'&&!expectedTableOrderId&&activeTable.length)return res.status(409).json({error:'This table is now active. Review the offline order before merging it.',code:'table_changed',conflict:tableConflict});
    if (activeTable.length && source!=='captain') return res.status(409).json({ error:'This table is already active. Add items from the Orders console to keep one bill and one KOT flow.' });
    ...
    if (activeTable.length && source==='captain') {
      ...
      saved.forEach((item)=>{const match=merged.find((current)=>current.name===item.name&&current.category===item.category&&current.portion===item.portion&&current.style===item.style&&String(current.note||'')===String(item.note||''));if(match)match.quantity=Math.min(20,Number(match.quantity||0)+Number(item.quantity||0));else merged.push(item);});
      const nextTotal=merged.reduce((sum,item)=>sum+Number(item.quantity||0)*(priceNumber(item.price)+(item.style?10:0)),0)-Number(activeTable[0].loyalty_points_redeemed||0)*loyalty.pointValue;
      await sql`UPDATE direct_orders SET items=${JSON.stringify(merged)},total=${nextTotal},special_request=CASE WHEN ${String(specialRequest||'').trim()}='' THEN special_request WHEN special_request='' THEN ${String(specialRequest||'').trim().slice(0,240)} ELSE special_request || ' · ' || ${String(specialRequest||'').trim().slice(0,240)} END,updated_at=NOW() WHERE id=${activeTable[0].id}`;
      await recordOrderEvent(activeTable[0].id,'captain-items-added',{itemCount:saved.reduce((count,item)=>count+Number(item.quantity||0),0),source:'captain',captainId:captain.id,captainName:captain.name});
      return res.status(201).json({id:activeTable[0].id,status:activeTable[0].status,orderNumber:String(activeTable[0].daily_order_number).padStart(2,'0'),total:nextTotal,continued:true});
    }
    if (requestedLoyaltyPoints) {
      await ensureLoyaltyTable();
    ...
    await recordOrderEvent(id, 'created', { source:captain?'captain':isDineIn?'dine-in':'counter', status:initialStatus, fulfillment, tableArea:isDineIn?dineInArea:'', tableNumber:isDineIn?dineInNumber:null, loyaltyRedeemed:requestedLoyaltyPoints, loyaltyEarned:earned, itemCount:saved.reduce((count,item)=>count+Number(item.quantity||0),0), ...(captain?{captainId:captain.id,captainName:captain.name}:{}) });
...
app.get('/api/orders', async (req, res) => { try { await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable()]); const search=String(req.query.search||'').replace(/\D/g,'').slice(0,16); const like=`%${search}%`; const today=kolkataOrderDay(); const history=req.query.history==='1'; const requestedDay=/^\d{4}-\d{2}-\d{2}$/.test(String(req.query.date||''))?String(req.query.date):''; const operatingStatus=await getOrdersOperatingStatus(); const select="SELECT o.*, (SELECT COUNT(*) FROM direct_orders h WHERE h.customer_phone=o.customer_phone) AS customer_order_count, (SELECT MAX(h.created_at) FROM direct_orders h WHERE h.customer_phone=o.customer_phone AND h.id<>o.id) AS customer_last_order_at, COALESCE((SELECT e.details->>'captainId' FROM order_events e WHERE e.order_id=o.id AND e.event_type='created' ORDER BY e.created_at ASC LIMIT 1),'') AS captain_id FROM direct_orders o"; res.set({ 'Cache-Control':'no-store', 'X-Orders-Day': today, 'X-Orders-View': history?'history':'current', 'X-Orders-Session': operatingStatus.open?'open':'closed' }); let orders; if (history && !requestedDay) orders=await sql(`${select} WHERE ($1='' OR o.customer_phone LIKE $2 OR CAST(o.daily_order_number AS TEXT) LIKE $2) ORDER BY o.created_at DESC LIMIT 100`,[search,like]); else if (history) orders=await sql(`${select} WHERE o.order_day=$1::date AND ($2='' OR o.customer_phone LIKE $3 OR CAST(o.daily_order_number AS TEXT) LIKE $3) ORDER BY o.created_at DESC LIMIT 100`,[requestedDay||today,search,like]); else orders=await sql(`${select} WHERE o.order_day=$1::date AND ($2='' OR o.customer_phone LIKE $3 OR CAST(o.daily_order_number AS TEXT) LIKE $3) ORDER BY o.created_at DESC LIMIT 100`,[today,search,like]); if(req.captain){const areas=Array.isArray(req.captain.areas)?req.captain.areas:[];orders=orders.filter((order)=>order.mode==='table'&&(!areas.length||areas.includes(String(order.table_area||''))));} res.json(orders); } catch (error) { res.status(500).json({ error: error.message }); } });
...
app.get('/api/orders/:id/print', async (req, res) => { try {
  await ensureDirectOrdersTable();
  await ensureLoyaltyTable();
  const rows=await sql`SELECT o.*, COALESCE(l.points, 0) AS loyalty_points FROM direct_orders o LEFT JOIN loyalty_accounts l ON l.customer_phone=o.customer_phone WHERE o.id=${req.params.id} LIMIT 1`;
...
app.post('/api/orders/:id/kots', async (req, res) => { try {
  await ensureDirectOrdersTable(); await ensureOperationsConfigTable(); await ensureKotsTable(); await ensureKotStationStatusTable(); await ensureKotRoundStatusTable();
  ...
  if (req.captain) {
    ...
    if (orderRows[0].mode!=='table' || (assignedAreas.length && !assignedAreas.includes(String(orderRows[0].table_area||'')))) return res.status(403).json({ error:'This order is not assigned to your Captain account.' });
  }
  ...
  const kotItemKey=(item)=>`${item.category||''}::${item.name||''}::${item.portion||''}::${item.style||''}::${item.note||''}`;
  const sent=new Map(); previous.forEach((kot)=>{ const quantities=new Map(); (Array.isArray(kot.tickets)?kot.tickets:[]).forEach((ticket)=>(Array.isArray(ticket.items)?ticket.items:[]).forEach((item)=>{ const key=kotItemKey(item); quantities.set(key,Math.max(quantities.get(key)||0,Number(item.quantity||0))); })); quantities.forEach((quantity,key)=>sent.set(key,(sent.get(key)||0)+quantity)); });
  const pending=(Array.isArray(orderRows[0].items)?orderRows[0].items:[]).map((item)=>{const key=kotItemKey(item);const quantity=Math.max(0,Number(item.quantity||0)-(sent.get(key)||0));return quantity?{...item,quantity}:null;}).filter(Boolean);
  ...
  for (const item of pending) {
    ...
    matchingRoutes.forEach((route)=>{ const printer=printers.find((candidate)=>candidate.id===route.printerId&&candidate.type==='kot'&&candidate.deviceName); if (!printer||printedBy.has(printer.id)) return; printedBy.add(printer.id); if(!groups.has(printer.id)) groups.set(printer.id,{ printerId:printer.id, printerName:printer.deviceName, printerLabel:printer.name, items:[] }); groups.get(printer.id).items.push(item); });
  }
  ...
  await Promise.all(tickets.map((ticket)=>Promise.all([
...
app.get('/api/orders/kitchen-statuses', async (req,res)=>{try{await ensureDirectOrdersTable();await ensureKotRoundStatusTable();const day=kolkataOrderDay();const rows=await sql`SELECT DISTINCT ON (s.order_id,s.printer_id) s.order_id,s.kot_number,s.printer_id,s.status,s.updated_at FROM order_kot_round_status s JOIN direct_orders o ON o.id=s.order_id WHERE o.order_day=${day}::date AND o.status IN ('accepted','preparing','ready') ORDER BY s.order_id,s.printer_id,s.kot_number DESC`;res.set('Cache-Control','no-store');res.json(rows)}catch(error){res.status(500).json({error:'Unable to load kitchen display statuses.'})}});
app.patch('/api/orders/:id/table', async (req,res)=>{try{
  await ensureDirectOrdersTable(); await ensureOperationsConfigTable();
  const tableArea=String(req.body?.tableArea||'').trim().slice(0,60), tableNumber=Number.parseInt(req.body?.tableNumber,10);
  if (!tableArea || !Number.isInteger(tableNumber) || tableNumber < 1 || tableNumber > 9999) return res.status(400).json({ error:'Choose a valid destination table.' });
  const today=kolkataOrderDay(); const [orderRows, configRows]=await Promise.all([sql`SELECT id,status,table_area,table_number FROM direct_orders WHERE id=${req.params.id} AND mode='table' AND order_day=${today}::date LIMIT 1`,sql`SELECT config FROM order_operations_config WHERE config_key='default' LIMIT 1`]);
  const order=orderRows[0]; if (!order) return res.status(404).json({ error:'Table order not found.' });
  if (!['saved','held','accepted','preparing','ready'].includes(order.status)) return res.status(409).json({ error:'Only an active table order can be moved.' });
  const areas=Array.isArray(configRows[0]?.config?.tableAreas)?configRows[0].config.tableAreas:[];
  if (!areas.some((area)=>String(area.name||'')===tableArea&&tableNumber>=Number(area.from)&&tableNumber<=Number(area.to))) return res.status(400).json({ error:'The destination table is not allocated in Operations.' });
  if (String(order.table_area)===tableArea && Number(order.table_number)===tableNumber) return res.json({ ok:true, tableArea, tableNumber, unchanged:true });
  const occupied=await sql`SELECT id FROM direct_orders WHERE mode='table' AND table_area=${tableArea} AND table_number=${tableNumber} AND order_day=${today}::date AND status IN ('saved','held','accepted','preparing','ready') AND id<>${req.params.id} LIMIT 1`;
...
app.patch('/api/orders/:id/kitchen-status/:printerId', async (req,res)=>{try{await ensureDirectOrdersTable();await ensureKotStationStatusTable();await ensureKotRoundStatusTable();const status=['accepted','preparing','ready'].includes(String(req.body?.status||''))?String(req.body.status):'',kotNumber=Number.parseInt(req.body?.kotNumber,10);const printerId=String(req.params.printerId||'').replace(/[^a-zA-Z0-9_-]/g,'').slice(0,80);if(!status||!printerId||!Number.isInteger(kotNumber)||kotNumber<1)return res.status(400).json({error:'Choose a valid KOT status update.'});const found=await sql`SELECT id FROM direct_orders WHERE id=${req.params.id} AND status NOT IN ('completed','rejected','cancelled') LIMIT 1`;if(!found.length)return res.status(404).json({error:'That active order was not found.'});await sql`INSERT INTO order_kot_round_status (order_id,kot_number,printer_id,status) VALUES (${req.params.id},${kotNumber},${printerId},${status}) ON CONFLICT (order_id,kot_number,printer_id) DO UPDATE SET status=EXCLUDED.status,updated_at=NOW()`;await sql`INSERT INTO order_kot_station_status (order_id,printer_id,status) VALUES (${req.params.id},${printerId},${status}) ON CONFLICT (order_id,printer_id) DO UPDATE SET status=EXCLUDED.status,updated_at=NOW()`;await recordOrderEvent(req.params.id,'kitchen-status',{printerId,kotNumber,status});res.json({ok:true,status,kotNumber})}catch(error){res.status(500).json({error:'Unable to update kitchen status.'})}});
...
app.put('/api/orders/operations', async (req, res) => { try {
  ...
  const printers=(Array.isArray(source.printers)?source.printers:[]).slice(0,250).map((printer)=>{
    ...
    return {
      ...
      billingMainWidth:layout(printer.billingMainWidth,160,400,280), billingOuterTop:layout(printer.billingOuterTop,0,40,0), billingOuterRight:layout(printer.billingOuterRight,0,40,0), billingOuterBottom:layout(printer.billingOuterBottom,0,40,0), billingOuterLeft:layout(printer.billingOuterLeft,0,40,0),
      ...
      serialColumnWidth:layout(printer.serialColumnWidth,0,40,10), quantityColumnWidth:layout(printer.quantityColumnWidth,8,60,20), priceColumnWidth:layout(printer.priceColumnWidth,15,100,40), amountColumnWidth:layout(printer.amountColumnWidth,15,120,55), itemRowGap:layout(printer.itemRowGap,0,20,5), separatorGap:layout(printer.separatorGap,0,20,5), separatorThickness:layout(printer.separatorThickness,1,4,1), kotHeaderFontSize:layout(printer.kotHeaderFontSize,8,24,12), kotTitleFontSize:layout(printer.kotTitleFontSize,10,26,15), kotMetaFontSize:layout(printer.kotMetaFontSize,8,20,10), kotItemFontSize:layout(printer.kotItemFontSize,8,22,12), kotFooterFontSize:layout(printer.kotFooterFontSize,8,20,10), kotBottomFeedLines:layout(printer.kotBottomFeedLines,0,12,3), itemsPerPage:layout(printer.itemsPerPage,0,80,0)
    };
...
app.get('/api/admin/captains', async (req,res)=>{try{await ensureOperationsConfigTable();const [config,rows]=await Promise.all([getSection('captain'),sql`SELECT config FROM order_operations_config WHERE config_key='default' LIMIT 1`]);const areas=Array.isArray(rows[0]?.config?.tableAreas)?rows[0].config.tableAreas.map((area)=>String(area.name||'').trim()).filter(Boolean):[],idleMinutes=Math.max(2,Math.min(120,Number(config.settings?.idleMinutes)||15));res.json({captains:(Array.isArray(config.captains)?config.captains:[]).map(({pinHash,...captain})=>({...captain,pinConfigured:!!pinHash})),areas,settings:{idleMinutes}});}catch(error){res.status(500).json({error:'Unable to load Captain accounts.'});}});
app.get('/api/admin/captains/activity', async (req,res)=>{try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable(),ensureOperationsConfigTable()]);const day=kolkataOrderDay(),[events,config,operations]=await Promise.all([sql`SELECT e.event_id,e.event_type,e.created_at AS event_at,e.details,o.id AS order_id,o.daily_order_number,o.table_area,o.table_number,o.status,o.items,o.total,o.service_state,o.service_requested_at FROM order_events e JOIN direct_orders o ON o.id=e.order_id WHERE o.order_day=${day}::date AND e.event_type IN ('created','captain-items-added','table-service-request','kot-served') AND COALESCE(e.details->>'captainId','')<>'' ORDER BY e.created_at DESC LIMIT 500`,getSection('captain'),sql`SELECT config FROM order_operations_config WHERE config_key='default' LIMIT 1`]);const captains=(Array.isArray(config.captains)?config.captains:[]).map((captain)=>({id:String(captain.id||''),name:String(captain.name||''),active:captain.active!==false})).filter((captain)=>captain.id&&captain.name),areas=(Array.isArray(operations[0]?.config?.tableAreas)?operations[0].config.tableAreas:[]).map((area)=>String(area.name||'').trim()).filter(Boolean);res.set('Cache-Control','no-store');res.json({day,events,captains,areas});}catch(error){res.status(500).json({error:'Unable to load Captain activity.'});}});
app.put('/api/admin/captains', async (req,res)=>{try{await ensureOperationsConfigTable();const [existing,rows]=await Promise.all([getSection('captain'),sql`SELECT config FROM order_operations_config WHERE config_key='default' LIMIT 1`]);const availableAreas=new Set((Array.isArray(rows[0]?.config?.tableAreas)?rows[0].config.tableAreas:[]).map((area)=>String(area.name||'').trim()).filter(Boolean)),idleMinutes=Math.max(2,Math.min(120,Number(req.body?.settings?.idleMinutes)||15));const old=new Map((Array.isArray(existing.captains)?existing.captains:[]).map((captain)=>[captain.id,captain]));const submitted=Array.isArray(req.body?.captains)?req.body.captains:[];if(submitted.length>50)return res.status(400).json({error:'Too many Captain accounts.'});const captains=submitted.map((entry)=>{const id=String(entry.id||crypto.randomUUID()).replace(/[^a-zA-Z0-9_-]/g,'').slice(0,64),name=String(entry.name||'').trim().slice(0,60),areas=[...new Set((Array.isArray(entry.areas)?entry.areas:[]).map((area)=>String(area).trim().slice(0,60)).filter(Boolean))],pin=String(entry.pin||'');if(areas.some((area)=>!availableAreas.has(area)))throw new Error('Choose assigned areas from the current table setup.');if(!id||!name)throw new Error('Every Captain needs a name.');if(pin&&!/^\d{4,6}$/.test(pin))throw new Error('PINs must contain 4 to 6 digits.');const previous=old.get(id);if(!previous?.pinHash&&!pin)throw new Error(`Set a PIN for ${name}.`);const pinHash=pin?crypto.scryptSync(pin,`captain:${id}`,64).toString('hex'):previous.pinHash;return{id,name,areas,active:entry.active!==false,pinHash};});await saveSection('captain',{captains,settings:{idleMinutes}});res.json({captains:captains.map(({pinHash,...captain})=>({...captain,pinConfigured:!!pinHash})),settings:{idleMinutes}});}catch(error){res.status(400).json({error:error.message||'Unable to save Captain accounts.'});}});
const captainSessionSecret=process.env.CAPTAIN_SESSION_SECRET||process.env.ADMIN_PASSWORD||crypto.randomBytes(32).toString('hex');
const captainSession=(captain)=>{const payload=Buffer.from(JSON.stringify({id:captain.id,name:captain.name,areas:captain.areas||[],exp:Date.now()+12*60*60*1000})).toString('base64url');return `${payload}.${crypto.createHmac('sha256',captainSessionSecret).update(payload).digest('base64url')}`;};
const readCaptainSession=(token)=>{try{const[payload,signature]=String(token||'').split('.'),expected=crypto.createHmac('sha256',captainSessionSecret).update(payload||'').digest('base64url');if(!payload||!signature||signature.length!==expected.length||!crypto.timingSafeEqual(Buffer.from(signature),Buffer.from(expected)))return null;const data=JSON.parse(Buffer.from(payload,'base64url').toString());return data.exp>Date.now()&&data.id&&data.name?data:null;}catch{return null;}};
const getActiveCaptainSession=async(token)=>{const session=readCaptainSession(token);if(!session)return null;try{const config=await getSection('captain'),captain=(config.captains||[]).find((entry)=>entry.id===session.id&&entry.active!==false&&entry.pinHash);return captain?{...session,name:captain.name,areas:captain.areas||[]}:null}catch{return null;}};
app.get('/api/captain/accounts',async(req,res)=>{try{const config=await getSection('captain');res.set('Cache-Control','no-store');res.json({captains:(Array.isArray(config.captains)?config.captains:[]).filter((captain)=>captain.active!==false&&captain.pinHash).map(({id,name,areas})=>({id,name,areas:areas||[]}))});}catch(error){res.status(500).json({error:'Unable to load Captain accounts.'});}});
app.post('/api/captain/login',async(req,res)=>{if(!allowPublicRequest(req,res,'captain-login',8,5*60*1000))return;try{const config=await getSection('captain'),captain=(config.captains||[]).find((entry)=>entry.id===String(req.body?.id||'')),idleMinutes=Math.max(2,Math.min(120,Number(config.settings?.idleMinutes)||15));const pin=String(req.body?.pin||''),stored=captain?.pinHash?Buffer.from(captain.pinHash,'hex'):null,attempt=captain?.id?crypto.scryptSync(pin,`captain:${captain.id}`,64):null;if(!captain||!captain.active||!stored||stored.length!==attempt.length||!crypto.timingSafeEqual(stored,attempt))return res.status(401).json({error:'Incorrect PIN.'});res.set('Cache-Control','no-store');res.json({captain:{id:captain.id,name:captain.name,areas:captain.areas||[],idleMinutes},token:captainSession(captain)});}catch(error){res.status(500).json({error:'Unable to sign in.'});}});
app.get('/api/captain/menu-insights',async(req,res)=>{const captain=await getActiveCaptainSession(req.get('X-Captain-Session'));if(!captain)return res.status(401).json({error:'Captain sign-in has expired. Sign in again.'});try{await ensureDirectOrdersTable();const rows=await sql`SELECT item->>'name' AS name,item->>'category' AS category,SUM(COALESCE((item->>'quantity')::numeric,0))::integer AS quantity FROM direct_orders o CROSS JOIN LATERAL jsonb_array_elements(COALESCE(o.items,'[]'::jsonb)) item WHERE o.created_at >= NOW()-INTERVAL '30 days' AND o.status NOT IN ('rejected','cancelled') GROUP BY item->>'name',item->>'category' ORDER BY quantity DESC,name ASC LIMIT 12`;res.set('Cache-Control','no-store');res.json({periodDays:30,items:rows.map((row)=>({name:String(row.name||''),category:String(row.category||''),quantity:Number(row.quantity||0)})).filter((row)=>row.name&&row.quantity>0)});}catch(error){res.status(500).json({error:'Unable to load menu insights.'})}});
app.get('/api/captain/ready-alerts',async(req,res)=>{const captain=await getActiveCaptainSession(req.get('X-Captain-Session'));if(!captain)return res.status(401).json({error:'Captain sign-in has expired. Sign in again.'});try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable(),ensureKotsTable(),ensureKotRoundStatusTable()]);const day=kolkataOrderDay(),alerts=await sql`SELECT o.id,o.daily_order_number,o.table_area,o.table_number,o.updated_at,latest.kot_number,MAX(s.updated_at) AS ready_at FROM direct_orders o JOIN order_events e ON e.order_id=o.id AND e.event_type='created' AND e.details->>'captainId'=${captain.id} JOIN (SELECT order_id,MAX(COALESCE(daily_kot_number,kot_number)) AS kot_number FROM order_kots GROUP BY order_id) latest ON latest.order_id=o.id JOIN order_kot_round_status s ON s.order_id=o.id AND s.kot_number=latest.kot_number WHERE o.order_day=${day}::date AND o.mode='table' AND o.status IN ('accepted','preparing','ready') GROUP BY o.id,o.daily_order_number,o.table_area,o.table_number,o.updated_at,latest.kot_number HAVING COUNT(*)>0 AND COUNT(*) FILTER (WHERE s.status='ready')=COUNT(*) ORDER BY ready_at DESC LIMIT 20`;res.set('Cache-Control','no-store');res.json({alerts});}catch(error){res.status(500).json({error:'Unable to load ready-to-serve alerts.'});}});
app.post('/api/captain/orders/:id/kots/:kotNumber/served',async(req,res)=>{const captain=await getActiveCaptainSession(req.get('X-Captain-Session')),kotNumber=Number.parseInt(req.params.kotNumber,10);if(!captain)return res.status(401).json({error:'Captain sign-in has expired. Sign in again.'});if(!Number.isInteger(kotNumber)||kotNumber<1)return res.status(400).json({error:'Invalid KOT round.'});try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable(),ensureKotRoundStatusTable()]);const owner=await sql`SELECT o.id FROM direct_orders o JOIN order_events e ON e.order_id=o.id AND e.event_type='created' AND e.details->>'captainId'=${captain.id} WHERE o.id=${req.params.id} AND o.mode='table' AND o.status IN ('accepted','preparing','ready') LIMIT 1`;if(!owner.length)return res.status(404).json({error:'That active table order is not assigned to this Captain.'});const served=await sql`UPDATE order_kot_round_status SET status='served',updated_at=NOW() WHERE order_id=${req.params.id} AND kot_number=${kotNumber} AND status='ready' RETURNING printer_id`;if(!served.length)return res.status(409).json({error:'This KOT round is not ready to serve.'});await recordOrderEvent(req.params.id,'kot-served',{kotNumber,captainId:captain.id,captainName:captain.name,printerCount:served.length});res.json({ok:true,kotNumber})}catch(error){res.status(500).json({error:'Unable to mark this KOT round served.'})}});
app.get('/api/captain/orders/:id/kot-progress',async(req,res)=>{const captain=await getActiveCaptainSession(req.get('X-Captain-Session'));if(!captain)return res.status(401).json({error:'Captain sign-in has expired. Sign in again.'});try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable(),ensureKotRoundStatusTable()]);const owner=await sql`SELECT o.id FROM direct_orders o JOIN order_events e ON e.order_id=o.id AND e.event_type='created' AND e.details->>'captainId'=${captain.id} WHERE o.id=${req.params.id} AND o.mode='table' AND o.status IN ('accepted','preparing','ready') LIMIT 1`;if(!owner.length)return res.status(404).json({error:'That active table order is not assigned to this Captain.'});const rounds=await sql`SELECT kot_number,COUNT(*)::integer AS stations,COUNT(*) FILTER (WHERE status='preparing')::integer AS preparing,COUNT(*) FILTER (WHERE status='ready')::integer AS ready,COUNT(*) FILTER (WHERE status='served')::integer AS served FROM order_kot_round_status WHERE order_id=${req.params.id} GROUP BY kot_number ORDER BY kot_number DESC LIMIT 8`;res.set('Cache-Control','no-store');res.json({rounds:rounds.map((round)=>({kotNumber:Number(round.kot_number),stations:Number(round.stations||0),preparing:Number(round.preparing||0),ready:Number(round.ready||0),served:Number(round.served||0)}))});}catch(error){res.status(500).json({error:'Unable to load kitchen progress.'})}});
app.post('/api/captain/orders/:id/service',async(req,res)=>{const captain=await getActiveCaptainSession(req.get('X-Captain-Session')),serviceState=['bill_requested','assistance_requested','water_requested'].includes(String(req.body?.serviceState||''))?String(req.body.serviceState):'';if(!captain)return res.status(401).json({error:'Captain sign-in has expired. Sign in again.'});if(!serviceState)return res.status(400).json({error:'Choose a valid table service request.'});try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable()]);const owner=await sql`SELECT o.id FROM direct_orders o JOIN order_events e ON e.order_id=o.id AND e.event_type='created' AND e.details->>'captainId'=${captain.id} WHERE o.id=${req.params.id} AND o.mode='table' AND o.status IN ('accepted','preparing','ready') LIMIT 1`;if(!owner.length)return res.status(404).json({error:'That active table order is not assigned to this Captain.'});await sql`UPDATE direct_orders SET service_state=${serviceState},service_requested_at=NOW(),updated_at=NOW() WHERE id=${req.params.id}`;await recordOrderEvent(req.params.id,'table-service-request',{serviceState,captainId:captain.id,captainName:captain.name});res.json({ok:true,serviceState})}catch(error){res.status(500).json({error:'Unable to send the table service request.'})}});
app.patch('/api/orders/:id/service',async(req,res)=>{const serviceState=['active','cleaning','available'].includes(String(req.body?.serviceState||''))?String(req.body.serviceState):'';if(!serviceState)return res.status(400).json({error:'Choose a valid service state.'});try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable()]);const updated=await sql`UPDATE direct_orders SET service_state=${serviceState},service_requested_at=CASE WHEN ${serviceState}='active' THEN NULL ELSE service_requested_at END,updated_at=NOW() WHERE id=${req.params.id} AND mode='table' AND status NOT IN ('completed','rejected','cancelled') RETURNING id`;if(!updated.length)return res.status(404).json({error:'That active table order was not found.'});await recordOrderEvent(req.params.id,'table-service-updated',{serviceState});res.json({ok:true,serviceState})}catch(error){res.status(500).json({error:'Unable to update table service.'})}});

app.post('/api/captain/orders/:id/move',async(req,res)=>{const captain=await getActiveCaptainSession(req.get('X-Captain-Session')),tableArea=String(req.body?.tableArea||'').trim().slice(0,60),tableNumber=Number.parseInt(req.body?.tableNumber,10);if(!captain)return res.status(401).json({error:'Captain sign-in has expired. Sign in again.'});if(!tableArea||!Number.isInteger(tableNumber)||tableNumber<1)return res.status(400).json({error:'Choose a valid destination table.'});try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable(),ensureOperationsConfigTable()]);const owner=await sql`SELECT o.table_area,o.table_number FROM direct_orders o JOIN order_events e ON e.order_id=o.id AND e.event_type='created' AND e.details->>'captainId'=${captain.id} WHERE o.id=${req.params.id} AND o.mode='table' AND o.status IN ('saved','held','accepted','preparing','ready') LIMIT 1`;if(!owner.length)return res.status(404).json({error:'That active table order is not assigned to this Captain.'});const config=await sql`SELECT config FROM order_operations_config WHERE config_key='default' LIMIT 1`,areas=config[0]?.config?.tableAreas||[];if(!areas.some((area)=>String(area.name)===tableArea&&tableNumber>=Number(area.from)&&tableNumber<=Number(area.to)))return res.status(400).json({error:'Choose an allocated table.'});const occupied=await sql`SELECT id FROM direct_orders WHERE mode='table' AND table_area=${tableArea} AND table_number=${tableNumber} AND status IN ('saved','held','accepted','preparing','ready') AND id<>${req.params.id} LIMIT 1`;if(occupied.length)return res.status(409).json({error:'That table already has an active order.'});await sql`UPDATE direct_orders SET table_area=${tableArea},table_number=${tableNumber},updated_at=NOW() WHERE id=${req.params.id}`;await recordOrderEvent(req.params.id,'table-moved',{fromArea:owner[0].table_area,fromNumber:Number(owner[0].table_number),toArea:tableArea,toNumber:tableNumber,captainId:captain.id});res.json({ok:true,tableArea,tableNumber})}catch(error){res.status(500).json({error:'Unable to move this table.'})}});
...
app.get('/api/admin/health', async (req, res) => {
  ...
  try {
    ...
    checks.ordersConsole = process.env.ORDERS_USERNAME && process.env.ORDERS_PASSWORD
