...
app.patch('/api/orders/:id/table', async (req,res)=>{try{
  ...
  const [orderRows, configRows]=await Promise.all([sql`SELECT id,status,table_area,table_number FROM direct_orders WHERE id=${req.params.id} AND mode='table' LIMIT 1`,sql`SELECT config FROM order_operations_config WHERE config_key='default' LIMIT 1`]);
