...
app.get('/api/admin/captains/activity', async (req,res)=>{try{await Promise.all([ensureDirectOrdersTable(),ensureOrderEventsTable()]);const day=kolkataOrderDay();const events=await sql`SELECT e.event_id,e.event_type,e.created_at AS event_at,e.details,o.id AS order_id,o.daily_order_number,o.table_area,o.table_number,o.status,o.items,o.total,o.service_state,o.service_requested_at FROM order_events e JOIN direct_orders o ON o.id=e.order_id WHERE o.order_day=${day}::date AND o.status NOT IN ('completed','rejected','cancelled') AND e.event_type IN ('created','captain-items-added','table-service-request','kot-served') AND COALESCE(e.details->>'captainId','')<>'' ORDER BY e.created_at DESC LIMIT 500`;res.set('Cache-Control','no-store');res.json({day,events});}catch(error){res.status(500).json({error:'Unable to load Captain activity.'});}});
