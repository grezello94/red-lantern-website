...
function setupCustomerInsights() {
  ...
  const load = async () => { try { status.textContent='Loading customer history…'; const response=await fetch(`/api/admin/customer-insights?date=${encodeURIComponent(date.value)}&search=${encodeURIComponent(search.value)}`,{cache:'no-store'}); const data=await response.json(); if(!response.ok)throw new Error(data.error||'Unable to load customer insights.'); stats.innerHTML=[['Orders',data.summary.orders],['Order value',money(data.summary.total)],['Points in wallets',data.summary.points],['Credit outstanding',money(data.summary.credit)]].map(([label,value])=>`<div class="insight-stat"><strong>${esc(value)}</strong><span>${esc(label)}</span></div>`).join(''); rows.innerHTML=data.orders.length?data.orders.map((order)=>{const items=Array.isArray(order.items)?order.items.reduce((sum,item)=>sum+Number(item.quantity||0),0):0;const placed=new Date(order.created_at).toLocaleString('en-IN',{dateStyle:'medium',timeStyle:'short'});return `<tr><td><strong>#${esc(String(order.daily_order_number||'—').padStart(2,'0'))}</strong></td><td>${esc(placed)}</td><td><strong>${esc(order.customer_name||'Guest')}</strong><br>${esc(order.customer_phone)}</td><td>${items} items</td><td><strong>${money(order.total)}</strong></td><td><span class="insight-pill">${Number(order.loyalty_points||0)} points</span></td><td><span class="insight-pill ${Number(order.credit_balance||0)>0?'credit':''}">${money(order.credit_balance||0)}</span></td><td><span class="insight-pill">${esc(order.status)}</span></td></tr>`;}).join(''):'<tr><td colspan="8">No orders match this date or search.</td></tr>'; leaderboard.innerHTML=data.leaderboard.length?data.leaderboard.map((customer,index)=>`<div class="customer-leader"><span>#${index+1} · ${esc(customer.customer_phone)}</span><b>${Number(customer.points||0)} points</b><span>Earned ${Number(customer.total_earned||0)} · Redeemed ${Number(customer.total_redeemed||0)}</span></div>`).join(''):'<p class="help-text">No loyalty points have been earned yet.</p>'; status.textContent=`Showing ${data.orders.length} order${data.orders.length===1?'':'s'}.`; }catch(error){status.textContent=error.message;status.style.color='#b91c1c';} };
