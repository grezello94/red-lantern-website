...
function addonGroupMarkup(group) { const options=Array.isArray(group.options)?group.options:[]; return `<article class="air-addon-group" data-addon-group="${escapeHtml(group.id)}"><div class="addon-group-head"><div><b>${escapeHtml(group.name||'New add-on group')}</b><small>Set rules now; link this group to dishes later.</small></div><button type="button" data-remove-addon-group="${escapeHtml(group.id)}" class="remove-air-item" aria-label="Remove add-on group">×</button></div><div class="addon-fields"><div class="form-group"><label>Group name</label><input data-addon-field="name" value="${escapeHtml(group.name||'')}" placeholder="e.g. Extras"></div><div class="form-group"><label>Online display name</label><input data-addon-field="displayName" value="${escapeHtml(group.displayName||'')}" placeholder="e.g. Choose your extras"></div></div><div class="addon-rules"><div class="form-group"><label>Minimum selections</label><input data-addon-field="min" type="number" min="0" max="20" value="${Number(group.min||0)}"></div><div class="form-group"><label>Maximum selections</label><input data-addon-field="max" type="number" min="1" max="20" value="${Number(group.max||1)}"></div><div class="form-group"><label>Selection type</label><select data-addon-field="selection"><option value="single" ${group.selection==='single'?'selected':''}>Single selection</option><option value="multiple" ${group.selection==='multiple'?'selected':''}>Multiple selection</option></select></div><div class="switch-row"><div><strong>Active</strong><span class="help-text" style="margin:4px 0 0">Available when assigned</span></div><input data-addon-field="active" type="checkbox" ${group.active!==false?'checked':''}></div></div><div><strong>Addon items</strong><div class="addon-options"><div class="addon-options-head"><span>Option name</span><span>Price</span><span>Dietary</span><span></span></div><div data-addon-options>${options.map((option,index)=>`<div class="addon-option"><input data-addon-option="name" data-option-index="${index}" value="${escapeHtml(option.name||'')}" placeholder="e.g. Extra cheese"><input data-addon-option="price" data-option-index="${index}" type="number" min="0" step="0.01" value="${Number(option.price||0)}" placeholder="0"><select data-addon-option="dietary" data-option-index="${index}"><option value="veg" ${option.dietary!=='nonveg'?'selected':''}>Veg</option><option value="nonveg" ${option.dietary==='nonveg'?'selected':''}>Non-Veg</option></select><button type="button" data-remove-addon-option="${index}" class="remove-air-item" aria-label="Remove option">×</button></div>`).join('')||'<p class="addon-empty">No options yet. Add the choices guests can select.</p>'}</div></div><button type="button" data-add-addon-option class="addon-add-option">+ Add option</button></div></article>`; }
function syncAirAddonGroups() { document.getElementById('air-addon-groups').value=JSON.stringify(airAddonGroups); }
function renderAirAddonGroups(groups=airAddonGroups) { airAddonGroups=Array.isArray(groups)?groups.map((group)=>({...group,id:group.id||addonId(),options:Array.isArray(group.options)?group.options:[]})):[]; const list=document.getElementById('air-addon-groups-list'); if(!list)return; list.innerHTML=airAddonGroups.map(addonGroupMarkup).join('')||'<p class="air-empty">No addon groups yet. Add groups such as Extras, Sauces, or Preparation choices.</p>'; syncAirAddonGroups(); }
function readAirAddonGroup(element) { const id=element.dataset.addonGroup; const group=airAddonGroups.find((item)=>item.id===id); if(!group)return; group.name=element.querySelector('[data-addon-field="name"]')?.value.trim()||''; group.displayName=element.querySelector('[data-addon-field="displayName"]')?.value.trim()||''; group.min=Math.max(0,Math.min(20,Number(element.querySelector('[data-addon-field="min"]')?.value)||0)); group.max=Math.max(1,Math.min(20,Number(element.querySelector('[data-addon-field="max"]')?.value)||1)); group.selection=element.querySelector('[data-addon-field="selection"]')?.value==='multiple'?'multiple':'single'; group.active=!!element.querySelector('[data-addon-field="active"]')?.checked; group.options=[...element.querySelectorAll('[data-addon-option="name"]')].map((input,index)=>({name:input.value.trim(),price:Math.max(0,Number(element.querySelector(`[data-addon-option="price"][data-option-index="${index}"]`)?.value)||0),dietary:element.querySelector(`[data-addon-option="dietary"][data-option-index="${index}"]`)?.value==='nonveg'?'nonveg':'veg'})).filter((option)=>option.name); syncAirAddonGroups(); }
...
document.addEventListener('change',(event)=>{const card=event.target.closest('[data-addon-group]');if(card)readAirAddonGroup(card);});
